    ...
    _createClass(DesktopNavigation, [{
      ...
      value: function _setupInlineNavigation() {
        ...
        this.element.querySelectorAll('.mega-menu').forEach(function (megaMenu) {
          ...
          megaMenu.style.left = (megaMenu.getBoundingClientRect().left * -1) + "px"
        ...
        function alignMegaMenu() {
          _this5.element.querySelectorAll('.mega-menu').forEach(function (megaMenu) {
            megaMenu.style.left = (megaMenu.closest(".nav-bar__linklist").getBoundingClientRect().left * -1) + "px"
            console.log(megaMenu.closest(".nav-bar__linklist").getBoundingClientRect().left)
          });
        }

        window.addEventListener("resize", function(){alignMegaMenu(this.element)});
  ...
  var Cart = /*#__PURE__*/function () {
    function Cart(element, options) {
      ...
      if (window.theme.pageType !== 'cart' && this.miniCartElement) {
        this.miniCartToggleElement = document.querySelector("[aria-controls=\"".concat(this.miniCartElement.id, "\"]"));
    ...
    _createClass(Cart, [{
      ...
      key: "_rerender",
      value: async function _rerender() {
        if (window.giftsThreshold) {
          try {
            const res = await fetch('/cart.json');
            const cart = await res.json();
            const gift = cart.items.find((item) => item.properties._gift);
            if (gift) {
              const threshold = window.giftsThreshold[gift.properties._gift];
              if (typeof threshold === 'number' && (cart.total_price - gift.final_line_price) < threshold) {
        ...
        var skipRecommendations = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
        ...
        if (window.theme.pageType !== 'cart') {
          ...
          if (!skipRecommendations) {
            const lastProduct = this.miniCartElement.querySelector('[data-product-id]');
            if (lastProduct && lastProduct.getAttribute('data-product-id')) {
              url = `/recommendations/products?product_id=${lastProduct.getAttribute('data-product-id')}&section_id=mini-cart`;
            } else if (lastProductAdded) {
              url = `/recommendations/products?product_id=${lastProductAdded}&section_id=mini-cart`;
            }
          }
        } else {
        ...
        return fetch(url, {
          ...
          console.error(err);
          this._rerender(scrollToTop, true);
        });
      }
      /**
      ...
      value: function _onWindowClick(event) {
        if (this.miniCartElement &&
            ...
            this.isMiniCartOpen &&
            ((!this.miniCartToggleElement.contains(event.target) && !this.miniCartElement.contains(event.target) && !event.target.closest('#variant__add-to-cart'))
            || event.target.closest('[data-action="close-mini-cart"]'))) {
          this._closeMiniCart();
        }
      }
    }]);
  ...
  var ProductVariants = /*#__PURE__*/function () {
    ...
    _createClass(ProductVariants, [{
      ...
      value: function _addToCart(event, target) {
        ...
        lastProductAdded = formElement.querySelector('[name="product-id"]').value;
      ...
      (function () {
        ...
        fastdom.mutate(function () {
          for (var i = 0, linksLength = links.length; i < linksLength; i++) {
            if (links[i].hostname !== window.location.hostname && links[i].target !== "_self") {
